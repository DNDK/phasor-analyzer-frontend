  ...
  try {
    ...
    if (e instanceof ApiError) {
      if (e.status === 400 || e.status === 422) {
        errorMessage.value = e.message || 'Проверьте введённые данные'
      } else if (e.status === 409) {
        errorMessage.value = 'Пользователь с таким именем или email уже существует'
      } else {
        errorMessage.value = `Ошибка сервера (${e.status}): ${e.message}`
      }
    } else {
  ...
  <div class="min-h-dvh flex items-center justify-center bg-gradient-to-br from-sky-50 to-cyan-50 p-6">
    <div class="w-full max-w-md">
      <div class="text-center mb-10">
        ...
        <h1 class="text-3xl font-bold text-slate-900">Создать аккаунт</h1>
        <p class="text-slate-500 mt-2">Зарегистрируйтесь, чтобы начать анализ</p>
      </div>
      ...
      <div class="rounded-3xl border border-slate-100 bg-white shadow-xl shadow-sky-100 p-8">
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="username (3–50 символов, a-z 0-9 _-)"
              class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition"
            />
          </div>
        ...
        <p class="text-center text-sm text-slate-500 mt-6">
          Уже есть аккаунт?
          <RouterLink to="/login" class="font-semibold text-sky-700 hover:text-sky-900">
            Войти
          </RouterLink>
        </p>
      </div>
